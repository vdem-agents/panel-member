# build_signeddev_by_condition.R — does movement / prominence's effect on signed deviation
# (AI rating - panel mean; positive = too generous, negative = too harsh) hold up across the
# de-identification ladder? Same simplification as build_slope_by_condition.R (2026-09-06): two
# SEPARATE models, no interaction term, run across all four base conditions + the trimmed
# Fine-Tuned pair (Codebook, Evidence). Simpler than the difficulty-slope version -- no case
# difficulty (h) term at all, since signed deviation isn't itself regressed against difficulty
# here, just against the moderator directly:
#
#   signed_dev ~ movement_signed   (movement-only; drops reid)
#   signed_dev ~ reid              (reid-only; drops movement)
#
# Movement uses the SIGNED, rank-tamed version (sign(dpoly_signed) * percentile rank of
# |dpoly_signed|) -- direction is the whole point for signed deviation (lag/anchoring test: does
# a backslid country still get an overly generous rating), unlike the difficulty-slope test where
# only magnitude mattered. A negative coefficient here means lag/anchoring: the AI's rating still
# leans toward the pre-2018 level rather than the current one.
#
# Usage:
...
# 1's convention — including the "Raw Text" label instead of "Evidence" (2026-09-06).
...
build_signeddev_by_condition <- function(proj_root,
  ...
  ai <- run_files |> map(read_run) |> bind_rows() |>
    ...
    mutate(signed_dev = rating - raw_mean)

  movement <- readRDS(file.path(proj_root, "data", "derived", glue("movement_{year}.rds")))$dpoly |>
    mutate(movement_value = sign(dpoly_signed) *
             rank(abs(dpoly_signed), na.last = "keep") / sum(!is.na(dpoly_signed)))
  ...
  fit_one <- function(model_key, family, cond_value) {
    d <- filter(ai, model_key == !!model_key, condition == cond_value) |>
      select(country_text_id, indicator, signed_dev)
    ...
    d <- filter(d, !is.na(signed_dev), !is.na(.data[[mod_col]]))
  ...
  if (write) {
    ...
    path <- file.path(out_dir, glue("signeddevbycondition_{moderator}_{year}.rds"))
    saveRDS(bundle, path)
    message(glue("signed-deviation-by-condition ({moderator}) bundle written: {path} · {nrow(effects)} rows"))
  }
  invisible(bundle)
}
...
if (sys.nframe() == 0) {
  ...
  build_signeddev_by_condition(proj_root, moderator = opt$moderator, year = opt$year)
}
